...
import { enqueueFinancialAnalysisTask } from '../../../../lib/cloud-tasks'
import {
  hasSameOrigin,
  hasValidCsrfToken,
...
const logger = createLogger('web-analysis')
...
export async function POST(request: NextRequest) {
  try {
    ...
    const task = await enqueueFinancialAnalysisTask({
    ...
    logger.info(
      ...
      'Financial analysis generation enqueued',
    )
    ...
    logger.error(
      ...
      'Financial analysis generation enqueue failed',
    )
    return NextResponse.json(
      { error: 'financial-analysis-enqueue-failed' },
      { status: 502 },
    )
  }
}
